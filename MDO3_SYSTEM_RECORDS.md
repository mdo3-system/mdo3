...
| **WebAssembly配信** | `/wasm/calc_core.wasm` | **合格 (200 OK)** | MIMEタイプ `application/wasm` 正常送出確認。 |
| **旧URL転送/認証保護** | `/calc/wall_4split` | **合格 (302 Found)** | 未ログイン時は `/login` へ安全リダイレクト。ログイン後は `/index.html` へ転送。 |

---

## 5. Phase 2 確定仕様 (ツール体系 & 料金プラン)
...
1. **基礎・擁壁・地盤系 (8ツール)**:
   ...
   - 逆Ｌ型・逆Ｔ型擁壁の計算
   - Ｌ型擁壁の計算（2.0m未満）
   - **土砂災害特別警戒区域の外壁等** *(※本カテゴリに配備)*
2. **木造軸組・接合部・一般部材系 (7ツール)**:
   - 柱の曲げ計算
   - めり込み補強計算
   - 横架材のZ低減係数
   - 屋根葺き材等の検討
...
### ② 確定料金体系 (Pricing & Stripe Plans)
| プラン区分 | 料金 (税込) | 対象範囲・特徴 |
| :--- | :--- | :--- |
| **① ツール個別プラン** | **980円 / 月** | 各ツール単体での月額サブスクリプション。必要な計算書だけをピンポイント利用。 |
| **② カテゴリ別パック** | **1,980円 / 月** | 4大カテゴリ（「基礎・擁壁」「木造軸組」「水平構面・詳細」「WRC造」）ごとの使い放題。 |
| **③ 全ツール使い放題** | **3,980円 / 月**<br>*(年額: **39,800円 / 年**)* | 全28ツール（今後追加される新ツール含む）の完全無制限利用。年額は実質2ヶ月分お得。 |
| **④ 操作説明動画・マニュアル** | **完全無償 (0円)** | **全ツールの実務操作動画・機能マニュアル・入力仕様を無料公開**。<br>「何ができて何ができないか」を事前に確認できる判断基準コンテンツとして提供。 |
