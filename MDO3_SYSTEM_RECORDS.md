...
## 15. リリース履歴: v1.5.0 (2026-09-25)

### ① ヘッダーメニューのスマート化 (折り返し解消 ＆ 無償バッジ付与)
...
- **構成**:
  ...
  3. **有効基礎梁成・dt 自動算出ツール (無償 / 審査対応 / ARCHITREND連携)**:
     - 主筋2段配筋時の重心距離 $dt$ および有効梁成 $d$ をリアルタイム算定。
     - 粗骨材あき $\max(31.25\text{mm}, 1.5d)$ を厳密考慮。
     - 木造住宅構造計算規準（グレー本）の「一律70mm」規定に対する審査機関の質疑回答・安全側丸め値を自動判定。
     - 計算根拠ワンクリックコピー機能配備。
...
- **構成**:
  - 横一列の5枠（CSS Grid 5カラム）:
    - 枠1: 基礎・擁壁・地盤系 (8ツール) → 3Dフリップ・コロコロ切り替え
    - 枠2: 木造軸組・接合部系 (5ツール) → 3Dフリップ・コロコロ切り替え
    - 枠3: 水平構面・耐力壁系 (5ツール) → 3Dフリップ・コロコロ切り替え
    - 枠4: WRC造パッケージ (5ツール) → 3Dフリップ・コロコロ切り替え
    - 枠5: CAD連携・斜め壁Web-CAD (AZツール 1ツール固定ハイライト)
